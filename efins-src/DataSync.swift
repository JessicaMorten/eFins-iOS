                ...
                if(success) {
                    ...
                    self.push({
                        ...
                        dispatch_async(dispatch_get_main_queue(),{
                            ...
                            self.delegate?.dataSyncDidStartPhotos?()
                            // TRB 6/14/16 - Disabling photo push for pilot #2 since it was onloy half-done anyway
...
//    }
            ...
            .responseString{ response in
                self.queueThreadTask { () -> () in
                    ...
                    if ((response.result.error) != nil) {
                        self.log("Connection Error, Problem connecting to server: \(response.result.error). \(response)")
                        ...
                        self.log(response.result.value!)
                    }
                    continuation(false)
                }
        }
    ...
    func push(continuation: () -> ()) {
        ...
        for (key, model) in Models {
            ...
            //print("tmp array size \(count(tempArray))")
            ...
            .responseString{ response in
                self.queueThreadTask { () -> () in
                    if (response.result.error != nil) {
                        ...
                        self.deleteOrUpdateLocalObjects(response.result.value!)
                        continuation()
                        return
                    } else {
    ...
    func digestResults(json: JSON) -> Bool {
        ...
        try! self.syncRealm!.commitWriteTransaction()
        let end = NSDate()
    ...
    func deleteOrUpdateLocalObjects(jsonString: String) -> Bool {
        ...
        for (key, subJson): (String, JSON) in json {
            for (nkey, newInfo): (String, JSON) in subJson {
                ...
                if queryResults?.count <= 0 {
                    ...
                    let newModelObject = joker?.init(value: modelObject) as! EfinsModel
        ...
        try! dRealm.commitWriteTransaction()
